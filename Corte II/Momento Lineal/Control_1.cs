...
public class Control_1 : MonoBehaviour
{
    ...
    void Start() {
        ...
        Sphere_1.GetComponent<Sphere_1>().setVelocidad(new Vector3(velocidad_s1, 0, 0));
        Sphere_2.GetComponent<Sphere_1>().setVelocidad(new Vector3(velocidad_s2, 0, 0));
    }

    void Update() {
        float distancia = Mathf.Abs(Sphere_1.position.x - Sphere_2.position.x);
        float aux = 1.0f / (masa_s1 + masa_s2);
        Vector3 vel1 = Sphere_1.GetComponent<Sphere_1>().getVelocidad();
        Vector3 vel2 = Sphere_2.GetComponent<Sphere_1>().getVelocidad();
