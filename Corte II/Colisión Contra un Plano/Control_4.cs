...
public class Control_4 : MonoBehaviour {
    ...
    float ang = 0.0f;
    float vx1 = 2.0f, vy1 = 0.0f, vx2 = 0.0f, vy2 = 0.0f;
    float px1 = 0.0f, py1 = 0.0f, px2 = 4.0f, py2 = 0.16f;
    float m1 = 1.0f, m2 = 1000.0f;
    float e = 1.0f;
    float radio_s = 0.5f;

    void Start() {
        Sphere = this.gameObject.transform.GetChild(0);
        Quad = this.gameObject.transform.GetChild(1);

        Sphere.position = new Vector3(px1, py1, 0);
        Quad.position = new Vector3(px2, py2, 0);
        Sphere.GetComponent<Sphere_4>().setVelocidad(new Vector3(vx1, vy1, 0));
        Quad.GetComponent<Sphere_4>().setVelocidad(new Vector3(vx2, vy2, 0));
    }
    ...
    void Update() {
        ...
        float distancia = Vector3.Distance(Sphere.position, Quad.position);

        Vector3 vel1 = Sphere.GetComponent<Sphere_4>().getVelocidad();
        Vector3 vel2 = Quad.GetComponent<Sphere_4>().getVelocidad();
